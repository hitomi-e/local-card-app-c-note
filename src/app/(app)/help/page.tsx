...
type Method = { title: string; steps: string[] };
type UsageStep = {
...
const USAGE_STEPS: UsageStep[] = [
  {
    ...
    methods: [
      {
        title: 'C-Note QRコードから新規登録',
        steps: [
          'C-NoteのQRコードを読み取る',
          'メールアドレスとパスワード（英数8文字以上）を入力する',
          'メールが届くので「メールアドレスを認証する」をタップ',
          '手順2で入力したメールアドレスとパスワードを入力してログイン',
        ],
      },
      {
        title: 'C-Noteデジタル名刺QRから新規登録',
        steps: [
          '相手のデジタル名刺QRコードを読み取る',
          'メールアドレスとパスワード（英数8文字以上）を入力する',
          'メールが届くので「メールアドレスを認証する」をタップ',
          '手順2で入力したメールアドレスとパスワードを入力してログイン',
          '相手のデジタル名刺が表示されるので「フリーメモ」と「カテゴリー設定」を入力し、「名刺を登録する」をタップ',
          '相手のデジタル名刺が名刺帳へ登録される',
        ],
      },
    ],
    ...
    methods: [
      {
        ...
        steps: [
          'アプリ右下の「My名刺」をタップ、もしくはアプリ右上の自分のアイコンをタップして「My QRコード」を画面に表示させる',
          '相手に「My QRコード」を読み取らせる',
          '相手のアプリへ「My名刺」が表示されるので、相手が「フリーメモ」と「カテゴリー設定」を入力して「名刺を登録する」をタップする',
          '相手のアプリの名刺帳へ、自分のデジタル名刺が登録される',
        ],
      },
      {
        title: '相手の名刺を受け取る場合',
        steps: [
          'アプリ右上の自分のアイコンをタップして「QRコードスキャン」をタップする',
          '相手のデジタル名刺が表示されるので「フリーメモ」と「カテゴリー設定」を入力し、「名刺を登録する」をタップ',
          '相手のデジタル名刺が名刺帳へ登録される',
        ],
      },
